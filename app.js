...
async function loadMatches(coords=null){
  ...
  document.getElementById("facilities").innerHTML=list.map(f=>`
    <article class="facility">
      ...
      <h3>${f.name}</h3><div class="type">${f.type} • ${f.area}</div>
      <p><b>Services:</b> ${f.services}</p>
      <p><b>Timings:</b> ${f.timings}</p>
      <p><b>Accessibility:</b> ${f.accessibility}</p>
      <div class="tags"><span class="tag">🗣 ${f.languages}</span>${f.distance_km!==null?`<span class="tag">📍 ${f.distance_km} km</span>`:""}<span class="tag">Public facility</span></div>
    </article>`).join("");
}
